...
def save_animations(results, length=10, directory=None, show=False, parallel=True, **kwargs):
    ...
    if parallel:
        args = [(res, length, '{}animation_{}.mp4'.format(directory, r), show) 
                    for r, res in enumerate(results)]
        pool = mp.Pool(mp.cpu_count() - 1)
        pool.starmap_async(save_animation, args)
        pool.close()
        pool.join()
    else:
        for r, res in enumerate(results):
            save_animation(res, length, '{}animation_{}.mp4'.format(directory, r), show)


def save_animation(result, length=10, filename=None, show=False, **kwargs):
    """ Takes in a list of Result objects and creates and saves an animation of the 
        evolution of the pdf over time of total duration length seconds. """
...
#                 print(points, dat, pltpoints, pltdata)    
            plot = ax.plot_trisurf(points[:,0], points[:,1], dat, 
                cmap=plt.get_cmap('jet'), vmin=zmin, vmax=zmax)
        ...
        def update_plot(framenum, data, plot):
            ...
            for points,dat in zip(pltpoints, data[framenum]):
                plot = ax.plot_trisurf(points[:,0], points[:,1], dat, 
                    linewidth=0, cmap=plt.get_cmap('jet'), vmin=zmin, vmax=zmax)
            ax.set_xlim3d(bbox.bounds[0])
            ax.set_xlabel('$s_1$')
            ax.set_ylim3d(bbox.bounds[1])
            ax.set_ylabel('$s_2$')
        ...
        pdf_ani = animation.FuncAnimation(fig, update_plot, frames, 
                        fargs=(pltdata, plot), interval=interval, blit=False)
...
def save_animations_NIPS2(res1, res2, length=10, filename=None, show=False, **kwargs):
    ...
    for points,dat in zip(pltpoints1, pltdata1[0]):
        plot1 = ax1.plot_trisurf(points[:,0], points[:,1], dat,
                cmap=plt.get_cmap('jet'), vmin=zmin1, vmax=zmax1)
    for points,dat in zip(pltpoints2, pltdata2[0]):
        plot2 = ax2.plot_trisurf(points[:,0], points[:,1], dat, 
                cmap=plt.get_cmap('jet'), vmin=zmin2, vmax=zmax2)
        
    for ax,bbox,zmax,title in zip([ax1,ax2], [bbox1,bbox2], 
                                  [zmax1,zmax2], kwargs.get('titles')):
        # Setting the axes properties
        ax.set_xlim3d(bbox.bounds[0])
        ax.set_xlabel('$s_1$')
        ax.set_ylim3d(bbox.bounds[1])
    ...
    def update_plots(framenum, data, plot, titles=kwargs.get('titles')):
        ...
        for ax,bbox,zmax,title in zip([ax1,ax2], [bbox1,bbox2], [zmax1,zmax2], titles):
            # Setting the axes properties
            ax.set_xlim3d(bbox.bounds[0])
...
def save_animations_ICML(res1, res2, length=10, filename=None, show=False, **kwargs):
    """ Takes in a list of Result objects and creates and saves an animation of the 
        evolution of the pdf over time of total duration length seconds. """
    # save all result animations (for now)
    T = res1.problem.T
    ...
    for ax,bbox,zmax,title in zip([ax1,ax2], [bbox1,bbox2],
                                  [np.maximum(zmax1,zmax2)]*2, kwargs.get('titles')):
        # Setting the axes properties
        ax.set_xlim3d(bbox.bounds[0])
        ax.set_xlabel('$s_1$')
        ax.set_ylim3d(bbox.bounds[1])
